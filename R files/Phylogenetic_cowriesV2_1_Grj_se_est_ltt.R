    ...
    tbl = read.table(file = "Phylogenetic_cowriesV2_1_Grj_se_est_ltt.txt",header = T)
    pdf(file='Phylogenetic_cowriesV2_1_Grj_se_est_ltt.pdf',width=12, height=9)
    time = -tbl$time
    library(scales)
    plot(time,tbl$diversity, type="n",ylab= "Number of lineages", xlab="Time (Ma)", main="Range-through diversity through time", ylim=c(0,129.0),xlim=c(min(time),0))
